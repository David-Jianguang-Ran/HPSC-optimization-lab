...
#include "../esPIC/src/ContiguousArray.h"
...
void test_array_read() {
    ...
    for (int j = 1; j <= 10; ++j) {
        for (int i = 1; i <= 10; ++i) {
            if (test_obj_again.at(i, j) != (i - 1) + (j - 1) * 10) {
                std::cout << "[test_array_read] Test Failed, value should be " << ((i - 1) + (j - 1) * 10) << " but is:" << test_obj_again.at(i, j) << std::endl;
            }
        }
    }
}
...
void test_array_write() {
    ...
    for (int j = 1; j <= 10; ++j) {
        for (int i = 1; i <= 10; ++i) {
            test_obj_again.at(i,j) = (i - 1) + (j - 1) * 10;
        }
    }
